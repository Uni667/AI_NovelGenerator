...
CHARACTER_PLANNING_PROMPT = """
你是网文小说人物策划编辑。请根据项目设定，生成角色规划和高层剧情大纲。
## 项目设定
- 主题：{topic}
- 类型：{genre}
- 分类：{category}
...
## 输出要求
返回 JSON 对象，包含两个字段：
1. "characters"：数组，每个元素包括 name、description、first_appearance_chapter，类似人物建议格式。
2. "outline"：字符串，包含简要的章节级别剧情大纲，章节编号从 1 开始，每行形式 "章节号: 概要"。
只输出 JSON，不要额外解释或 Markdown。
"""
...
@router.post("/api/v1/projects/{project_id}/characters/plan")
def plan_characters(project_id: str, request: Request):
    """让 AI 根据当前设定和架构生成角色规划与高层剧情大纲，但不直接写入数据库。"""
    user_id = get_current_user(request)
    project = project_service.get_project(project_id, user_id)
    if not project:
        raise HTTPException(status_code=404, detail="项目不存在")
    ...
    prompt = CHARACTER_PLANNING_PROMPT.format(
        topic=pconfig.get("topic", ""),
        genre=pconfig.get("genre", ""),
        category=pconfig.get("category", ""),
        platform=pconfig.get("platform", ""),
    ...
    try:
        ...
        return {
            "characters": suggestions[:8],
            "outline": outline
        }
    except Exception as e:
        logger.exception("Character planning generation failed")
        raise HTTPException(status_code=500, detail="角色规划生成失败，请稍后重试")


# ── 角色库全景视图 ──

@router.post("/api/v1/projects/{project_id}/characters/refresh-from-file")
def refresh_characters_from_file(project_id: str, request: Request):
    """Parse character_state.txt and upsert characters, then return the updated list.
    Useful when the user manually edits the state file.
    """
    _check_project(project_id, request)
    # Perform file->DB sync
    sync_txt_to_db(project_id)
    # Return refreshed character list
    with get_db() as conn:
        rows = conn.execute(
            """SELECT * FROM character_profile WHERE project_id = ?
               ORDER BY CASE status WHEN 'appeared' THEN 0 WHEN 'planned' THEN 1 WHEN 'suggested' THEN 2 ELSE 3 END,
               COALESCE(first_appearance_chapter, 999999), updated_at DESC""",
            (project_id,)
        ).fetchall()
        return [dict(r) for r in rows]
