...
def create_project(data: dict) -> dict:
    ...
    os.makedirs(os.path.join(filepath, "角色库"), exist_ok=True)
    os.makedirs(os.path.join(filepath, "角色库", "全部"), exist_ok=True)

    with get_db() as conn:
        conn.execute(
            "INSERT INTO project (id, name, description, filepath, status, created_at, updated_at) VALUES (?,?,?,?,?,?,?)",
...
def update_project(project_id: str, data: dict) -> dict | None:
    ...
    with get_db() as conn:
        conn.execute(f"UPDATE project SET {', '.join(sets)} WHERE id=?", params)
    return get_project(project_id)
