...
import { Plus, Trash2, Edit3, Users, GitBranch, Loader2 } from "lucide-react"
...
export default function RelationshipManager({ projectId, characters }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await api.characterRelationships.update(projectId, editing.id, form)
      } else {
        await api.characterRelationships.create(projectId, form)
      }
  ...
  const handleDelete = async (relId: number) => {
    ...
    try {
      await api.characterRelationships.delete(projectId, relId)
      toast.success("已删除")
      load()
    } catch (e: any) {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">管理角色之间的结构化关系，为可视化留好基础</p>
        </div>
            ...
            <Card className="bg-muted/20">
              <CardContent className="py-3">
                <div className="flex flex-wrap gap-1 items-center text-sm">
                  ...
                  {graph.nodes.map(n => (
                    <Badge key={n.id} variant="outline" className="text-xs">{n.name}</Badge>
                  ))}
                </div>
              </CardContent>
            </Card>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-2 mt-1 text-xs text-muted-foreground">
                          <span>强度: {"★".repeat(Math.round(rel.strength * 5))}{"☆".repeat(5 - Math.round(rel.strength * 5))}</span>
                          <span>|</span>
                          <span>{STATUS_LABELS[rel.status] || rel.status}</span>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editing ? "编辑关系" : "新增关系"}</DialogTitle>
            <DialogDescription>定义两个角色之间的结构化关系</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-2">
              <div>
                <Label>角色 A</Label>
                <Select value={String(form.character_id_a)} onValueChange={v => setForm(f => ({ ...f, character_id_a: Number(v) }))}>
              ...
              <div>
                <Label>角色 B</Label>
                <Select value={String(form.character_id_b)} onValueChange={v => setForm(f => ({ ...f, character_id_b: Number(v) }))}>
                  <SelectTrigger><SelectValue placeholder="选择角色" /></SelectTrigger>
                  <SelectContent>
                    {characters.map(c => <SelectItem key={c.id} value={String(c.id)}>{c.name}</SelectItem>)}
                  </SelectContent>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <Label>关系类型</Label>
                <Select value={form.rel_type} onValueChange={v => setForm(f => ({ ...f, rel_type: v }))}>
                  <SelectTrigger><SelectValue placeholder="选择类型" /></SelectTrigger>
                  <SelectContent>
                    {Object.entries(RELATIONSHIP_TYPE_LABELS).map(([k, v]) => <SelectItem key={k} value={k}>{v}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div>
                ...
                <Select value={form.direction} onValueChange={v => setForm(f => ({ ...f, direction: v }))}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="a_to_b">A→B</SelectItem>
                    <SelectItem value="b_to_a">B→A</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
            <div>
              <Label>描述</Label>
              <Textarea rows={3} value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} placeholder="关系描述..." />
            </div>
            <div className="grid grid-cols-3 gap-2">
              <div>
                <Label>强度 ({form.strength.toFixed(1)})</Label>
                <Input type="range" min="0" max="1" step="0.1" value={form.strength} onChange={e => setForm(f => ({ ...f, strength: Number(e.target.value) }))} />
              </div>
              <div>
                <Label>起始章节</Label>
                <Input type="number" value={form.start_chapter ?? ""} onChange={e => setForm(f => ({ ...f, start_chapter: e.target.value ? Number(e.target.value) : null }))} placeholder="可选" />
              </div>
              <div>
                <Label>状态</Label>
