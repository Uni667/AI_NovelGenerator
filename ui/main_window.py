...
class NovelGeneratorGUI:
    ...
    def get_characters_involved_text(self) -> str:
        """Return the latest characters input from the textbox and keep the backing variable in sync."""
        if hasattr(self, "char_inv_text"):
            value = self.char_inv_text.get("0.0", "end").strip()
            if hasattr(self, "characters_involved_var"):
                self.characters_involved_var.set(value)
            return value
        return self.characters_involved_var.get().strip() if hasattr(self, "characters_involved_var") else ""
    ...
    def toggle_english_mode(self):
        ...
        try:
            if config_manager.IS_ENGLISH:
                self.english_mode_btn.configure(text="切换到中文模式")
                ...
                self.english_mode_btn.configure(text="切换到英文模式")
